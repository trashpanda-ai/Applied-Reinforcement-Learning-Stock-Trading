...
def main():
    ...
    for seed in range(3):
        for agent_name in ['a2c', 'ppo', 'ddpg', 'sac', 'td3']:
            ...
            saved_model_path = r"Evaluation/model/daily/normalized/Sentiment/threshold/" + agent_name + '/seed' + str(
                seed) + '.zip'
            ...
            # Display the plot
            plt.title(plot_overname + ' Heatmap of gradients')
            # plt.yscale("log")
            plt.xlabel('Feature Index')
            plt.ylabel('Hour')
            ensure_dir('Sensitivity/Heatmaps/daily/' + flagsentiment + agent_name)
            plt.savefig('Sensitivity/Heatmaps/daily/' + flagsentiment + agent_name + '/seed' + str(seed) + '.pdf')
            ...
            df_cats['Indicators'] = features[
                ['macd', 'boll_ub', 'boll_lb', 'rsi_30', 'cci_30', 'dx_30', 'close_30_sma', 'close_60_sma',
                 'vix']].mean(axis=1)
            df_cats['Sentiment'] = features[
                ['stocktwitsPosts', 'stocktwitsLikes', 'stocktwitsImpressions', 'stocktwitsSentiment']].mean(axis=1)
            ...
            plt.title(plot_overname + " Features")
            plt.xlabel("Index")
            plt.ylabel("Value")
            plt.show()
            ...
            plt.title(plot_overname + " CATEGORIES")
            plt.xlabel("Index")
            plt.ylabel("Value")
            plt.show()

            # Ensure directory exists before saving
            output_dir2 = 'Sensitivity/Category_importance/daily/'  + flagsentiment + agent_name
            ensure_dir(output_dir2)
            plt.savefig(output_dir2 + '/seed' + str(seed) + '.pdf')
            ...
            # Generate an array of 18 numbers between 0 and 1
            numbers = features.mean().values

            # Make some numbers bigger than others (for demonstration purposes)

            # ... you can continue this for other indices if needed

            labels = names

            # Ensure the length of labels matches the length of numbers
            assert len(labels) == len(numbers), "Mismatch between label count and number count"
            ...
            # Ensure directory exists before saving
            output_dir1 = 'Sensitivity/Feature_importance/daily/'  + flagsentiment + agent_name
            ensure_dir(output_dir1)
            plt.savefig(output_dir1 + '/seed' + str(seed) + '.pdf')
            ...
            plt.title(plot_overname + ' Sum of Gradients Over Time')
            plt.xlabel('Date')
            plt.ylabel('Sum of Values')
            plt.grid(True)
            plt.tight_layout()

            output_dir1 = 'Sensitivity/Day_importance/daily/'  + flagsentiment + agent_name
            ensure_dir(output_dir1)
            plt.savefig(output_dir1 + '/seed' + str(seed) + '.pdf')
            ...
            plt.title(plot_overname + ' Importance of Tics')
            plt.ylabel('Sum')
            ...
            output_dir1 = 'Sensitivity/Tic_importance/daily/'  + flagsentiment + agent_name
            ensure_dir(output_dir1)
            plt.savefig(output_dir1 + '/seed' + str(seed) + '.pdf')
